...
description: >
  ...
  "SAP_Template import JE bằng WB". Ưu tiên dùng khi user yêu cầu import
  Grab theo template JE mới thay vì format 1 sheet phẳng.
alwaysAllow:
  - Bash
  - Write
---

# Skill: GFB Import by JE
...
2. Đọc template `SAP_Template import JE bằng WB.xlsx` (ưu tiên bản đã đóng gói sẵn trong skill tại `data/template/`)
...
- Luôn tạo folder con `Output` trong thư mục làm việc của tháng và lưu toàn bộ file do skill tạo ra vào đó.
...
```bash
python "$VESPER_SKILL_DIR/scripts/convert_gfb_to_je_template.py" \
...
- Skill đã đóng gói sẵn template tại: `data/template/SAP_Template import JE bằng WB.xlsx`
